...
% using chore only
function [t,modeloutput] = glacier_response_chore(ks,a,b);
...
t0 = [1979,1993];
t = twglaciers(1).ocean.(product).t;
...
for i=1:length(twglaciers),
    ...
    % TF
    TF = interp1(twglaciers(i).ocean.(product).t,twglaciers(i).ocean.(product).TF_GL,t);
    % melt rate
    m(i,:) = ks*Q.^a.*TF.^b;
    m0(i) = mean(m(i,inds0));
    % melt rate Q varies
...
save(['modeloutput_',product,'.mat'],'t','modeloutput');
